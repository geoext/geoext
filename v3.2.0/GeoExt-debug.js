    ...
    statics: {
        ...
        getOgcFilterFromExtJsFilter: function(filters, type, combinator, wfsVersion) {
            if (!Ext.isDefined(filters) || !Ext.isArray(filters)) {
                Ext.Logger.error('Invalid filter argument given to ' + 'GeoExt.util.OGCFilter. You need to pass an array of ' + '"Ext.grid.filters.filter"');
                return;
            }
        ...
        getOgcFilterBodyFromExtJsFilterObject: function(filter, wfsVersion) {
            if (!Ext.isDefined(filter)) {
                Ext.Logger.error('Invalid filter argument given to ' + 'GeoExt.util.OGCFilter. You need to pass an instance of ' + '"Ext.grid.filters.filter"');
                return;
            }
            ...
            return GeoExt.util.OGCFilter.getOgcFilter(property, operator, value, wfsVersion);
        },
        /**
        ...
        getOgcFilter: function(property, operator, value, wfsVersion) {
            ...
            switch (operator) {
                ...
                case 'like':
                    value = '*' + value + '*';
                    var likeFilter = '<PropertyIsLike wildCard="*" singleChar="."' + ' escape="!" matchCase="false">' + '<' + propName + '>' + property + '</' + propName + '>' + '<Literal>' + value + '</Literal>' + '</PropertyIsLike>';
                    return likeFilter;
                case 'in':
                    ogcFilterType = 'Or';
                    ...
                    Ext.each(values, function(val) {
                        filters += '<PropertyIsEqualTo>' + '<' + propName + '>' + property + '</' + propName + '>' + '<Literal>' + val + '</Literal>' + '</PropertyIsEqualTo>';
                    });
            ...
            ogcFilterType = '<' + ogcFilterType + '>';
            closingTag = Ext.String.insert(ogcFilterType, '/', 1);
            var tpl = '' + '{0}' + '<' + propName + '>{1}</' + propName + '>' + '<Literal>{2}</Literal>' + '{3}';
        ...
        combineFilters: function(filters, combinator, omitNamespaces, wfsVersion) {
            ...
            parts.push('<Filter' + (omitNamespaces ? '' : ' xmlns="http://www.opengis.net/' + ns + '"') + '>');
            if (numFilters > 1) {
