...
Ext.theme = Ext.apply(Ext.theme || {}, {
   ...
   */
    addManifest: function (manifest) {
        ...
        let add = Ext.isArray(manifest) ? manifest : arguments;
   ...
   */
    addShortcuts: function (shortcuts) {
        ...
        for (const key in shortcuts) {
            ...
            let xtype = Ext.theme.addWidget(key);
   ...
   */
    applyShortcut: function (manifestEntry, shortcut) {
        ...
        const config = Ext.theme.copyProps({}, manifestEntry.config);
   ...
   */
    renderWidget: function (manifestEntry, shortcut) {
        ...
        const ct = Ext.fly(document.body).createChild({cls: 'widget-container'});
   ...
   */
    render: function () {
        ...
        let manifest = Ext.theme._manifest;
        var shortcuts = Ext.theme._shortcuts;

        for (let k = 0, n = manifest ? manifest.length : 0; k < n; ++k) {
            var manifestEntry = manifest[k];
            let xtype = Ext.theme.addWidget(manifestEntry.xtype);
            var widgetShortcuts = xtype ? shortcuts[xtype] : null;

            if (xtype && manifestEntry.ui && widgetShortcuts) {
                for (let i = 0; i < widgetShortcuts.length; i++) {
   ...
   */
    run: function () {
        const extjsVer = Ext.versions.extjs;
        var globalData = {};
        ...
        if (Ext.layout.Context) {
            Ext.override(Ext.layout.Context, {
                run: function () {
                    ...
                    var widget = Ext.theme.currentWidget;
                    if (!ok && widget) {
    ...
    // This function is defined by slicer.js (the framework-independent piece)
        var gsm = window && window['generateSlicerManifest'];
    let delta;
