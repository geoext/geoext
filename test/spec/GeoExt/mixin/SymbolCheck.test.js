  ...
  describe('Static methods', function () {
    describe('#check', function () {
      ...
      it('filled the cache (direct requires)', function () {
        const cache = GeoExt.mixin.SymbolCheck._checked;
      ...
      it('takes a short way out if no "symbols"', function () {
        const normalizeSymbolSpy = sinon.spy(
          GeoExt.mixin.SymbolCheck,
          'normalizeSymbol',
        );
        const checkSymbolSpy = sinon.spy(
          GeoExt.mixin.SymbolCheck,
          'checkSymbol',
        );
        const isDefinedSymbolSpy = sinon.spy(
          GeoExt.mixin.SymbolCheck,
          'isDefinedSymbol',
        );
      ...
      it('takes a shortcut for checked symbols', function () {
        ...
        const extEachSpy = sinon.spy(Ext, 'each');
        const extIsDefinedSpy = sinon.spy(Ext, 'isDefined');
        GeoExt.mixin.SymbolCheck.isDefinedSymbol(
          'Shub.Niggurath.Lord.Of.The.Wood',
        );
        ...
        expect(
          extIsDefinedSpy.calledWith(cache['Shub.Niggurath.Lord.Of.The.Wood']),
        ).to.be(true);
