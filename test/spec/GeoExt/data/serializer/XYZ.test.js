...
describe('GeoExt.data.serializer.XYZ', function () {
  ...
  describe('serializing behaviour', function () {
    ...
    it("doesn't throw on expected source", function () {
      expect(function () {
        GeoExt.data.serializer.XYZ.serialize(layer, source);
    ...
    it('serializes as expected', function () {
      const serialized = GeoExt.data.serializer.XYZ.serialize(
        layer,
        source,
        viewResolution,
      );
      const expected = {
        baseURL:
          'http://geo.nls.uk/maps/towns/' + 'glasgow1857/{z}/{x}/{-y}.png',
        opacity: 1,
        resolutions: [
          ...
          0.004665345964671402, 0.002332672982335701, 0.0011663364911678506,
          0.0005831682455839253, 0.00029158412279196264, 0.00014579206139598132,
          0.00007289603069799066, 0.00003644801534899533,
          0.000018224007674497665, 0.000009112003837248832,
          0.000004556001918624416, 0.000002278000959312208,
          0.000001139000479656104, 5.69500239828052e-7, 2.84750119914026e-7,
          1.42375059957013e-7, 7.11875299785065e-8, 3.559376498925325e-8,
        ],
        imageExtension: 'png',
        tileSize: [256, 256],
        type: 'OSM',
    ...
    it('throws if it cannot get an URL', function () {
      const sourceNoUrl = new ol.source.XYZ({});
      const layerNoUrlSource = new ol.layer.Tile({
        source: sourceNoUrl,
      });
