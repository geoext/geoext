describe('A GeoExt.data.model.print.Capability model', function () {
  const dataBasePath = typeof __karma__ === 'undefined' ? '' : 'base/test/';
  const url = dataBasePath + 'data/PrintCapabilities.json';
  let data;
  ...
  function loadFromRawData(props) {
    const recordProps = Ext.clone(props);
  ...
  before(function (done) {
    ...
    Ext.Ajax.request({
      ...
      failure: function (response) {
        Ext.raise('Could not retrieve test data. Code ' + response.status);
      },
      callback: function () {
        done();
      },
