...
describe('GeoExt.component.Map', function () {
  ...
  describe('public functions', function () {
    ...
    describe('getters and setters', function () {
      it('getCenter() returns an array with 2 numbers', function () {
      ...
      it('getExtent() returns an array with 4 numbers', function () {
      ...
      it(
        'getLayers() returns an ol.Collection containing' +
      ...
      it('getStore() returns a GeoExt.data.store.Layers instance', function () {
        expect(mapComponent.getStore()).to.be.an(GeoExt.data.store.Layers);
      });

      it('setCenter() sets the correct center', function () {
        const center = [1183893.8882437304, 7914041.721258021];
      ...
      it('setExtent() sets the correct center', function () {
        for (let i = 0; i < 10; i++) {
          ...
          const y1 = x1 + Math.random() * 100;
          const y2 = x2 + Math.random() * 100;
      ...
      it('setView() sets the correct ol.View to the ol.Map', function () {
    ...
    describe('layer handling', function () {
      it('addLayer() adds an ol.layer.Base to the ol.Map', function () {
      ...
      it('addLayer() throws error if no layer is passed', function () {
        const nolayer = [null, undefined, new ol.source.OSM(), '', map];

        nolayer.forEach(function (oneLayer) {
          expect(mapComponent.addLayer).withArgs(oneLayer).to.throwException();
        });
      });
      ...
      it('removeLayer() throws error if no layer is passed', function () {
        const nolayer = [null, undefined, new ol.source.OSM(), '', map];

        nolayer.forEach(function (oneLayer) {
          expect(mapComponent.removeLayer)
            .withArgs(oneLayer)
            .to.throwException();
        });
      });
    ...
    describe('listening to size changes', function () {
      it('ensure the map is updated when the size changes', function () {
        const spy = sinon.spy(map, 'updateSize');
    ...
    describe('pointerrest configuration', function () {
      ...
      it('can be turned on (config)', function () {
        const testObjs2 = TestUtil.setupTestObjects({
          mapComponentOpts: {
            pointerRest: true,
          },
        });
        const mapComponent2 = testObjs2.mapComponent;

        expect(mapComponent2.getPointerRest()).to.be(true);
        expect(mapComponent2.bufferedPointerMove).to.not.be(Ext.emptyFn);
      ...
      it('ensures bufferedPointerMove calls unbuffered one', function (done) {
        const spy = sinon.spy(mapComponent, 'unbufferedPointerMove');
        mapComponent.setPointerRestInterval(50);
        mapComponent.setPointerRest(true);

        // call into the generated bufferedPointerMove
        const mockOlEvt = {pixel: [0, 0]};
      ...
      it('ensures changing the interval changes the buffered method', function () {
        mapComponent.setPointerRest(true);
        const oldBuffered = mapComponent.bufferedPointerMove;
        mapComponent.setPointerRestInterval(50);
        const newBuffered = mapComponent.bufferedPointerMove;
