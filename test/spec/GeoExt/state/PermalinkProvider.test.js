...
describe('GeoExt.state.PermalinkProvider', function () {
  ...
  describe('public functions', function () {
    ...
    describe('readPermalinkHash()', function () {
      it('returns a correct state object', function () {
        const state = plProvider.readPermalinkHash(
    ...
    describe('getPermalinkHash()', function () {
      it('returns a correct URL hash', function () {
        const state = {
          zoom: 8,
        ...
        const hash = plProvider.getPermalinkHash();
        expect(hash).to.be('#map=8/-1061011.123456/6804570.9876/0');
      });

      it('rounds the coordinates if correspondig param is set', function () {
        const state = {
          zoom: 8,
          center: [-1061011.123456, 6804570.9876],
          rotation: 0,
        ...
        const hash = plProvider.getPermalinkHash(true);
        expect(hash).to.be('#map=8/-1061011.12/6804570.99/0');
      });
    });

    describe('set()', function () {
      it('keep our mapState object in sync with the state', function () {
        const state = {
          zoom: 8,
          center: [-1061011.123456, 6804570.9876],
          rotation: 0,
