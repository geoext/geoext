...
let featStore;
let wmsLayer;
let drawQueryInteraction;
let activeFilters = [];
const spatialOperators = Ext.create('Ext.data.Store', {
  fields: ['value', 'name'],
  data: [
    {
      value: 'intersect',
...
Ext.application({
      ...
      tools: [
        {
          ...
          onChange: function (val) {
            FeatureGridWithSpatialFilter.app.createDrawInteraction(val, true);
          },
        },
        {
          xtype: 'button',
          ...
          menu: [
            {
              text: 'Point',
              handler: function () {
                FeatureGridWithSpatialFilter.app.createDrawInteraction('Point');
              },
            },
            {
              text: 'LineString',
              handler: function () {
                FeatureGridWithSpatialFilter.app.createDrawInteraction(
                  'LineString',
                );
              },
            },
  ...
  refreshFilters: function () {
    const wmsFilter =
      GeoExt.util.OGCFilter.getOgcWmsFilterFromExtJsFilter(activeFilters);
  ...
  onDrawEnd: function (evt) {
    ...
    const spatialOperatorsCombo = Ext.ComponentQuery.query(
      'combobox[name="spatialOperatorsCombo"]',
    )[0];
    const operator = spatialOperatorsCombo.getValue();

    // Create an instance of the spatial filter
    const extFilter = GeoExt.util.OGCFilter.createSpatialFilter(
      operator,
      typeName,
      geometry,
  ...
  createDrawInteraction: function (geometryType, changedFromOperatorCombo) {
    ...
    drawQueryInteraction = new ol.interaction.Draw({
      ...
      geometryFunction:
        geometryType === 'bbox' ? ol.interaction.Draw.createBox() : undefined,
    });
    drawQueryInteraction.on(
      'drawend',
      FeatureGridWithSpatialFilter.app.onDrawEnd.bind(
