Ext.require(['Ext.panel.Panel', 'Ext.Viewport', 'GeoExt.component.Map']);
...
Ext.application({
  ...
  launch: function () {
    ...
    const permalinkDisplayPanel = Ext.create('Ext.panel.Panel', {
      title: 'Permalink',
      region: 'south',
      height: 100,
      border: false,
    ...
    Ext.create('Ext.Viewport', {
      layout: 'border',
      items: [mapPanel, description, permalinkDisplayPanel],
    });
    ...
    permalinkProvider.on({
      statechange: function (provider, name, value) {
        ...
        const hash = provider.getPermalinkHash();
        // assemble a permalink
        const plink = Ext.urlAppend(base, hash);
