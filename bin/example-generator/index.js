...
const APPNAME_PH = '@@EXAMPLE_NAME@@';
const APP_ROOT = __dirname + slash;
...
if (exampleId.indexOf('..') !== -1) {
    console.error(
        'Not allowed to provide a example name containing ".."! Exit...',
    );
    process.exit(1);
}
...
const pathExampleFolder = path.join(
  APP_ROOT,
  '..',
  '..',
  'examples',
...
const targetHtmlFile = path.join(
    APP_ROOT,
  '..',
  '..',
  'examples',
...
function replaceFileContent(file, template, replacement) {
    ...
    fs.readFile(file, 'utf8', function (err, data) {
        ...
        var result = data.replace(new RegExp(template, 'g'), replacement);
        // write back
